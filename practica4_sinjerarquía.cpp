...
int main()
{
	...
	*/
	...
	glm::mat4 modelaux(1.0);//Inicializar matriz de Modelo 4x4 auxiliar 
...
/*
	//para reiniciar la matriz de modelo con valor de la matriz identidad
		model = glm::mat4(1.0); //Reincio de matriz de modelo para descartar transformaciones anteriores
		//rotación alrededor de la articulación que une con la cabina
		model = glm::translate(model, glm::vec3(0.0f, 6.0f, -4.0f));												//Articulacion
		model = glm::rotate(model, glm::radians(mainWindow.getarticulacion1()), glm::vec3(0.0f, 1.0f, 0.0f));		//Articulacion sin dibujar figura
		...
		//Almacenar los valores en la matrix auxiliar

		//Descartar la scala que no se quiere heredad
		modelaux = model;
		model = glm::scale(model, glm::vec3(5.0f, 1.0f, 1.0f));
		...
		//model = glm::mat4(1.0); //Ya no se necesita descartar las translaciones
		//model = glm::translate(model, glm::vec3(0.0f, 6.0f, -4.0f));
		//model = glm::translate(model, glm::vec3(0.0f, -3.0f, 0.0f)); //Para que no quede acomodado en el centro, ya no se encima para que sea encimado y es de -2.5f
		...
		modelaux = model; //Para descartar la escala de la matriz anterior
...
/*		Ejerccicio 02		*/
		...
		color = glm::vec3(0.957f, 0.758f, 0.715f);
		...
		model = modelaux;
		//Oreja 1
		model = glm::translate(model, glm::vec3(0.0f, 1.5f, 0.95f));
		...
		glUniformMatrix4fv(uniformProjection, 1, GL_FALSE, glm::value_ptr(projection));
		glUniformMatrix4fv(uniformView, 1, GL_FALSE, glm::value_ptr(camera.calculateViewMatrix()));
		color = glm::vec3(0.277f, 0.191f, 0.176f);
		glUniform3fv(uniformColor, 1, glm::value_ptr(color)); //para cambiar el color del objetos
		...
		model = modelaux;
		//Ocico
		model = glm::translate(model, glm::vec3(1.0f, -1.9, 0.8f));
		...
		model = modelaux;
		//Cuello
		model = glm::translate(model, glm::vec3(0.0f, -2.0, -1.5f));
		...
		model = modelaux;
		//Arituculacion 1__ Se aumenta y disminuye con F y G
		...
		//Pata 1.1
		model = modelaux;
		model = glm::translate(model, glm::vec3(0.6f, -1.5f, -0.25f));
		...
		model = modelaux;
		//Pata 1.4
		model = glm::translate(model, glm::vec3(0.0f, -1.3f, 0.9));

		modelaux = model;
		model = glm::rotate(model, glm::radians(-15.0f), glm::vec3(1.0f, 0.0f, 0.0f));
		model = glm::scale(model, glm::vec3(1.2f, 0.5f, 2.0f));
		...
		model = modelaux;
		//Articulacion 2__ Se aumenta y disminuye con H y J
		model = glm::translate(model, glm::vec3(-2.0f, 5.8f, -1.9f));
		...
		//Pata 2.1
		model = modelaux;
		model = glm::translate(model, glm::vec3(-0.5f, -1.5f, 0.25f));
		model = glm::rotate(model, glm::radians(15.0f), glm::vec3(1.0f, 0.0f, 0.0f));
		...
		model = modelaux;
		//Arituculacion 3__ Se aumenta y disminuye con K y L
		...
		model = modelaux;
		//Articulacion 4__ Se aumenta y disminuye con H y J
		model = glm::translate(model, glm::vec3(-1.5f, 6.0f, -2.0f));
		model = glm::rotate(model, glm::radians(mainWindow.getarticulacion4()), glm::vec3(1.0f, 0.0f, 0.0f));
